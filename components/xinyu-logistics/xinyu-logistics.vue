			...
			<view>
				<view class="flex list">
					...
					<view class="info flex1">
						<view class="title address">广东省 东莞市 长安镇 某某某某某某某某某某某某地区</view>
					</view>
				</view>
				<view class="flex list" :class="{one: index == 0 && wlInfo.delivery_status == 1}" v-for="(item, index) in wlInfo.list"
				 :key="index">
					<view class="time">
					...
					<view class="info flex1">
						<view class="title">{{index == 0 && wlInfo.delivery_status == 1 ? '已签收' : ''}}</view>
						<view class="text">{{item.context}}
							<view class="texts">
									{{item.location}}
							</view>
						</view>
