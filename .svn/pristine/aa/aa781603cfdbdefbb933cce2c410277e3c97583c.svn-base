		...
		<template>
			<view class="footer" :style="{'padding-bottom': paddingBottomHeight + 'rpx'}">
				<view class="lfte">
					...
					<view class="item">
						<button @click="showPhone=true" show-message-card session-from send-message-path send-message-title
							:plain="true">
				...
				<phone-modal v-model="showPhone"></phone-modal>
			</view>
		</template>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getCommodityDetails() {
				this.$u.api.getCommodityDetails({
					...
					if(res.isActivity == 1){ //如果等于1则为活动商品，可以进行商品的数量增加
		...
		onLoad(obj) {
			...
			if(!obj.q){
				...
				let scan_url = 'https://qr.xianghuikejiapp.com?id=13617&encode=100031000';
				//扫码进入链接
