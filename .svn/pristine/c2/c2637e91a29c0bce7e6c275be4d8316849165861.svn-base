...
const api = {
	...
	payMonthly: "/om/api/wechatpay/payMonthly.do", // 使用月付支付
	beforePaying: "/om/api/wechatpay/beforePaying.do", // 先送后付
...
export function paymentApi(vm) {
	...
	let  payMonthly = (params = {}) => vm.$u.get(api.payMonthly, params);
	let  beforePaying = (params = {}) => vm.$u.get(api.beforePaying, params);
